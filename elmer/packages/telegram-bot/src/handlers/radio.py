...
Commands:
  ...
  /scan       — band scanner control: /scan [start|stop|pause|next]
  /tune       — tune to frequency: /tune <freq_khz>
  /band       — tune to band center: /band <band>
"""
...
_BAND_FREQS = {
    "10m": 28074, "12m": 24915, "15m": 21074, "17m": 18100,
    "20m": 14074, "40m": 7074, "80m": 3573,
}


async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/scan [start|stop|pause|next] — band scanner control."""
    await update.message.chat.send_action(ChatAction.TYPING)
    ...
    if action == "start":
        result = await _api_post("/radio/scanner/start")
        if result and result.get("ok"):
            order = result.get("scan_order", [])
            order_str = " → ".join(order)
            await update.message.reply_text(
                f"\U0001f4e1 Scanner started: {order_str}",
                parse_mode="Markdown",
            )
        else:
            err = result.get("error", "Unknown error") if result else "Core unreachable"
            await update.message.reply_text(f"\u274c Scanner start failed: {err}")
        return

    if action == "stop":
        result = await _api_post("/radio/scanner/stop")
        if result and result.get("ok"):
    ...
    if action == "pause":
        result = await _api_post("/radio/scanner/pause")
        if result and result.get("ok"):
            band = result.get("paused_on", "?")
            await update.message.reply_text(f"\u23f8\ufe0f Scanner paused on *{band}*",
                                            parse_mode="Markdown")
        else:
            err = result.get("error", "Unknown error") if result else "Core unreachable"
            await update.message.reply_text(f"\u274c {err}")
    ...
    if action == "resume":
        result = await _api_post("/radio/scanner/resume")
        if result and result.get("ok"):
    ...
    if action == "next":
        result = await _api_post("/radio/scanner/next")
        if result and result.get("ok"):
            await update.message.reply_text("\u23ed\ufe0f Skipping to next band.")
        else:
            err = result.get("error", "Unknown error") if result else "Core unreachable"
            await update.message.reply_text(f"\u274c {err}")
        return
    ...
    # Default: show status.
    data = await _api_get("/radio/scanner/status")
    if not data:
        await update.message.reply_text("\u26a0\ufe0f Could not reach scanner.")
        return

    if not data.get("scanning"):
        await update.message.reply_text(
            "\U0001f4e1 *Band Scanner*: not running\n"
            "Use `/scan start` to begin.",
            parse_mode="Markdown",
        )
        return

    band = data.get("current_band", "?")
    freq = data.get("current_frequency", 0)
    ...
    time_str = "Day" if is_day else "Night"

    # Mark current band in order.
    order_display = []
    for b in order:
        if b == band:
            order_display.append(f"[*{b}*]")
        else:
            order_display.append(b)

    order_str = " → ".join(order_display)
    lines = [
        f"\U0001f4e1 *Band Scanner* {status_emoji}",
        "",
        f"Band: *{band}* ({freq / 1000:.0f} kHz)",
        f"Remaining: {mins}m {secs:02d}s",
        f"Order: {order_str}",
        f"Cycles: {cycles} | {time_str}",
    ]

    if paused:
        lines.append("\n_Paused \u2014 use /scan resume_")
...
async def cmd_tune(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/tune <freq_khz> — manually tune to a frequency."""
    args = context.args or []
    if not args:
        await update.message.reply_text(
            "Usage: `/tune 14074` (frequency in kHz)", parse_mode="Markdown",
        )
        return

    try:
        freq_khz = float(args[0])
        freq_hz = int(freq_khz * 1000)
    except ValueError:
        await update.message.reply_text("Invalid frequency. Use kHz, e.g. `/tune 14074`",
                                        parse_mode="Markdown")
        return

    await update.message.chat.send_action(ChatAction.TYPING)
    result = await _api_post("/radio/frequency", json_data={"frequency_hz": freq_hz})
    if result and result.get("ok"):
        await update.message.reply_text(
            f"\U0001f4fb Tuned to *{freq_khz:.0f} kHz*", parse_mode="Markdown",
        )
    else:
        err = result.get("error", "Unknown error") if result else "Core unreachable"
        await update.message.reply_text(f"\u274c Tune failed: {err}")


# ── /band ────────────────────────────────────────────────────────────────


async def cmd_band(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/band <band> — tune to a band's FT8 center frequency."""
    args = context.args or []
    if not args:
        bands = ", ".join(_BAND_FREQS.keys())
        await update.message.reply_text(
            f"Usage: `/band 20m`\nAvailable: {bands}", parse_mode="Markdown",
        )
        return

    band_arg = args[0].lower()
    if not band_arg.endswith("m"):
        band_arg += "m"

    freq_khz = _BAND_FREQS.get(band_arg)
    if freq_khz is None:
        await update.message.reply_text(f"Unknown band: `{band_arg}`", parse_mode="Markdown")
        return
    ...
    if result and result.get("ok"):
        ...
        await _api_post("/radio/mode", json_data={"mode": mode})
        await update.message.reply_text(
            f"\U0001f4fb Tuned to *{band_arg}* ({freq_khz} kHz {mode})",
            parse_mode="Markdown",
        )
    else:
        err = result.get("error", "Unknown error") if result else "Core unreachable"
