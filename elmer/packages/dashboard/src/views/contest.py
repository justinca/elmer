"""Contest page — upcoming calendar, live dashboard, history, band advisor."""
...
def _render_data() -> None:
    ...
    tab_calendar, tab_live, tab_history, tab_bands = st.tabs(
        ["Upcoming", "Live Dashboard", "History", "Band Advisor"]
    )

    with tab_calendar:
        _render_calendar(api)
    ...
    with tab_bands:
        _render_band_advisor(api)


def _render_calendar(api: ElmerAPI) -> None:
    """Upcoming contest calendar."""
    days = st.slider("Look ahead (days)", 7, 365, 30, key="contest_days")
...
def _render_live(api: ElmerAPI) -> None:
    ...
    if st.button("Load Dashboard", type="primary", key="contest_load"):
        ...
        with r1:
            st.metric("Last 10 min", f"{rate_10.get('rate_per_hour', 0):.0f}/hr")
        with r2:
            st.metric("Last 60 min", f"{rate_60.get('rate_per_hour', 0):.0f}/hr")
        with r3:
...
def _render_history(api: ElmerAPI) -> None:
    """Historical contest participation from Log4OM."""
...
def _render_band_advisor(api: ElmerAPI) -> None:
    """Band change recommendation."""
    ...
    with col_band:
        current_band = st.selectbox(
            ...
            index=3,  # default to 20m
            key="band_advisor_band",
        )
    with col_contest:
        contest = st.text_input(
            "Contest (optional)",
            key="band_advisor_contest",
            placeholder="e.g. CQ-WW-CW",
        )

    if st.button("Get Recommendation", type="primary", key="band_advisor_btn"):
