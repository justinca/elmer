#!/usr/bin/env python3
"""Elmer WeatherPi health monitor agent.

Lightweight script designed to run on the WeatherPi (Raspberry Pi) as a
systemd service.  Publishes a heartbeat every 30 seconds via MQTT with:
  ...
  - weewx service status (weather station daemon)
  - meshtasticd service status (Meshtastic mesh radio)
...
# ---------------------------------------------------------------------------
NODE_NAME = os.environ.get("ELMER_NODE_NAME", "weatherpi")
...
def collect_metrics() -> dict:
    """Gather all health metrics for the WeatherPi."""
    ...
    # WeatherPi-specific services
    metrics["services"] = {
        "weewx": _service_status("weewx"),
        "meshtasticd": _service_status("meshtasticd"),
    }
